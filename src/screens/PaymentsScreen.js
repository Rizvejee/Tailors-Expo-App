  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      <FlatList
          ...
          <View>
            ...
            <View style={s.bigCard}>
              <Text style={s.bigCardTitle}>
                {now.toLocaleString('en-US', { month: 'long' })} {now.getFullYear()} — Monthly
              </Text>
              <View style={s.bigCardRow}>
                <StatItem label="Income"   value={monthIncome}  color={C.mid} />
                <StatItem label="Expenses" value={monthExp}     color={C.orange} />
                <StatItem label="Profit"   value={monthProfit}  color={monthProfit >= 0 ? C.green : C.danger} />
              </View>
            </View>

            {/* ── Card 2: This Year ── */}
            <View style={s.bigCard}>
              <Text style={s.bigCardTitle}>{now.getFullYear()} — Yearly</Text>
              <View style={s.bigCardRow}>
                <StatItem label="Income"   value={yearIncome}  color={C.mid} />
                <StatItem label="Expenses" value={yearExp}     color={C.orange} />
                <StatItem label="Profit"   value={yearProfit}  color={yearProfit >= 0 ? C.green : C.danger} />
              </View>
            </View>

            {/* ── Card 3: Remaining ── */}
            <View style={[s.bigCard, { borderLeftColor: C.gold, borderLeftWidth: 4 }]}>
              <Text style={s.bigCardTitle}>Total Remaining (All Orders)</Text>
              <Text style={[s.bigCardTitle, { fontSize: 20, fontWeight: '800', color: C.gold, marginTop: 4 }]}>
        ...
        renderItem={({ item }) => {
          const pct = item.price > 0 ? Math.round(((item.advance || 0) / item.price) * 100) : 0;
...
function StatItem({ label, value, color }) {
  return (
    <View style={{ flex: 1, alignItems: 'center' }}>
      <Text style={[s.statValue, { color }]}>Rs. {Math.abs(value).toLocaleString()}</Text>
      {value < 0 && <Text style={{ fontSize: 10, color: C.danger }}>Loss</Text>}
...
const s = StyleSheet.create({
  ...
  bigCard:           { backgroundColor: C.card, borderRadius: 16, padding: 12, marginHorizontal: 20, marginBottom: 10, elevation: 3, shadowColor: '#000', shadowOpacity: 0.07, shadowRadius: 8 },
  bigCardTitle:      { fontSize: 11, fontWeight: '700', color: C.textMid, textTransform: 'uppercase', letterSpacing: 0.4, marginBottom: 10 },
  bigCardRow:        { flexDirection: 'row', justifyContent: 'space-between' },
  statValue:         { fontSize: 14, fontWeight: '800', marginBottom: 2 },
  statLabel:         { fontSize: 10, fontWeight: '600', color: C.textLight, textTransform: 'uppercase', letterSpacing: 0.3 },

  sectionTitle:      { fontSize: 14, fontWeight: '700', color: C.dark, marginHorizontal: 20, marginBottom: 14, marginTop: 4 },
  payCard:           { backgroundColor: C.card, borderRadius: 16, padding: 16, marginHorizontal: 20, marginBottom: 12, elevation: 2, shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 6 },
